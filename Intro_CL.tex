...
\]
    ...
    \end{itemize}
    \end{enumerate}
    \item The search procedure stops iff $A$ is derivable
...
\begin{frame}
\frametitle{Soundness and completeness wrt the Tarski semantics}
 \begin{itemize}[<+->]   %  [<+-| alert@+>]
    \item Soundness easily proved by induction on $\Gamma\vdash_{\vec{y}}^T A$
    \item Not complete: $\emptyset\vdash_{\vec{\emptyset}}^{\forall x.\bot} p$
    underivable without a constant in the signature!
    \item So, let's assume a constant in the signature
...
\exists \vec{z_1}\Delta_1(\vec{y},\vec{z_1})\vee\dots\vee\exists \vec{z_n}\Delta
_n(\vec{y},\vec{z_n})
$$
