...
\frametitle{Automated reasoning (AR)}
 \begin{itemize}[<+->]   %  [<+-| alert@+>]
  ...
  \item We describe below how to eliminate function symbols
...
\frametitle{Elimination of function symbols}
 \begin{itemize}[<+->]
  ...
  a new $(n{+}1)$-predicates for an $n$-ary function, for example:
   ...
   where $C',D'$ are the result of the substitution in $C,D$.
...
\frametitle{Depth-first proof search in CL}
 \begin{itemize}[<+->]   %  [<+-| alert@+>]
    ...
    \begin{enumerate}
       ...
       \item Existential clauses (cause new variables)
       \item Disjunctive existential clauses (cause both, the worst)
    \end{enumerate}
