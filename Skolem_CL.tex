...
\mbox{ }\\
(jww Thierry Coquand, 2019,\\
DOI: {\tt 10.3233/FI-2019-1853})
}
...
\infer[\mathit{axiom}]{\Gamma}{\Gamma,B_1[\vec{t}/\vec{x}] \quad\cdots\quad \Gamma,B_n[\vec{t}/\vec{x}]}
    ...
    (In the forcing semantics, pairs like $(\vec{y};\Gamma)$ will be
...
\item Soundness easily proved by induction on $\Gamma\vdash_{\vec{y}}^T A$
    ...
    \item Proof of completeness (cf.\ tableaux, essentially non- constructive):
    Develop \red{fairly} the complete tree of possible derivations,
    stopping if $\bot$ or $A$ shows up. Infinite branches are models 
    of $\Gamma,\neg A$.
    \item Can be extended to arbitrary coherent conclusions $A$.
 \end{itemize}
\end{frame}
...
\begin{frame}
\frametitle{Two inevitable lemmas}
 \begin{itemize}[<+->]
    \item Weakening:
...
then $\Gamma,P(c)\vdash_{\vec{x}}^{T,\,P(c)} A$
by Lemma~\ref{weak}.
